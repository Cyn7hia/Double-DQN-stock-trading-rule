...
class MarketModelBuilder(AbstractModelBuilder):
	...
	def buildModel(self):
		...
		for i in xrange(1):
			...
			h = Conv2D(64, (hgt, 3), padding="valid")(S) # <tf.Tensor 'conv2d_1/BiasAdd:0' shape=(?, 2, 58, 64) dtype=float32>
			...
			h = Conv2D(128, (hgt, 5), padding="valid")(S)#(S) # <tf.Tensor 'conv2d_2/BiasAdd:0' shape=(?, 2, 56, 128) dtype=float32>
			h = LeakyReLU(0.001)(h)
			h = MaxPooling2D(pool_size=(2, 2), strides=None)(h)  # review

			h = Flatten()(h) # review
			merges1.append(h)  # review
			h = Conv2D(256, (hgt, 10), padding="valid")(S)#(S) # review: original
			h = LeakyReLU(0.001)(h) # review: original
			h = MaxPooling2D(pool_size=(2, 2), strides=None)(h)  # review

			h = Flatten()(h) # review
			merges1.append(h)  # review
			h = Conv2D(512, (hgt, 20), padding="valid")(S)#(S) # review: original
			h = LeakyReLU(0.001)(h) # review: original
			h = MaxPooling2D(pool_size=(2, 2), strides=None)(h)  # review

			h = Flatten()(h) # review
			merges1.append(h)  # review
			h = Conv2D(1024, (hgt, 40), padding="valid")(S)#(S) # review: original
			h = LeakyReLU(0.001)(h) # review: original
			h = MaxPooling2D(pool_size=(2, 2), strides=None)(h)  # review
			...
			h = Conv2D(2048, (hgt, 60), padding="valid")(S)
			h = LeakyReLU(0.001)(h)
