...
from market_env2 import MarketEnv
from market_model_builder import MarketModelBuilder
import os
import tensorflow as tf
#os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
# os.environ["CUDA_VISIBLE_DEVICES"] = "0"  # ly 
# config = tf.ConfigProto()  # ly
# config.gpu_options.per_process_gpu_memory_fraction = 0.6  # ly
# session = tf.Session(config=config)  # ly
...
if __name__ == "__main__":
    ...
    sgd = SGD(lr=0.001, decay=1e-6, momentum=0.9, nesterov=True) # review: !!must exist even when loading weights
    ...
    for e in range(epoch):
        ...
        while not game_over: #review: game_over = self.done in market_env
            ...
            if change:#change_tm1:#change:#change_tm1:
