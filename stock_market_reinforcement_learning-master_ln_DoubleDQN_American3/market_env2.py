...
class MarketEnv(gym.Env):
    ...
    def _step(self, action):
        ...
        #for i in xrange(len(self.boughts)):
            #self.boughts[i] = self.boughts[i] * MarketEnv.PENALTY * (1 + vari * (-1 if sum(self.boughts) < 0 else 1))#1.0 * MarketEnv.PENALTY * (1 + vari * (-1 if sum(self.boughts) < 0 else 1)) #
        ...
        self.Boughts.append(0.0) #used for continuous interest

        for i in xrange(len(self.boughts)):
            #self.boughts[i] = self.boughts[i] * MarketEnv.PENALTY * (1 + vari * (-1 if sum(self.boughts) < 0 else 1)) # for dicrete interest
            self.Boughts[i] = (self.Boughts[i] + vari * (-1 if sum(self.boughts) < 0 else 1)) * MarketEnv.PENALTY # for continuous interest
        #for b in self.boughts: #used for discrete interest
        for b in self.Boughts: #used for continuous interest
    ...
    def _steptest(self, action):
        ...
        for i in xrange(len(self.Boughts)):
            self.Boughts[i] = (self.Boughts[i] + vari * (-1 if sum(self.boughts) < 0 else 1)) * MarketEnv.PENALTY #used for continuous interest
